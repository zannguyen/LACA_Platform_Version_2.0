...
const RowButton = ({ icon, label }) => (
  <button type="button" className="st-row st-clickable" onClick={() => {}}>
...
export default function Setting() {
  ...
  const hint = useMemo(() => {
    ...
    if (permissionState === "denied") {
      return (
        <>
          Bạn đã chặn quyền vị trí ở trình duyệt. Hãy bật lại trong Site Settings
          để sử dụng Map/Home.
        </>
      );
    }
  ...
  const onToggleLocation = async (next) => {
    ...
    try {
      ...
      // Keep switch OFF if user denies or location fails.
      const msg =
        e?.code === 1
      ...
      <div className="st-content">
        <div className="st-section">
          ...
          <div className="st-row st-static">
            ...
            <label className={`st-switch ${busy ? "busy" : ""}`} aria-label="Allow location">
              <input
                type="checkbox"
                checked={enabled}
        ...
        <div className="st-section">
          ...
          <RowButton icon={<IconBlocked />} label="Blocked" />
          <RowButton icon={<IconShield />} label="Privacy and Data" />
        </div>

        <div className="st-section">
          <div className="st-section-title">Support</div>
          <RowButton icon={<IconMail />} label="Feedback" />
          <RowButton icon={<IconWarn />} label="Report issue" />
        </div>

        <div className="st-section">
          <div className="st-section-title">Account</div>
          <RowButton icon={<IconLogout />} label="Log out" />
          <RowButton icon={<IconTrash />} label="Delete account" />
        </div>
      </div>
    </div>
  );
